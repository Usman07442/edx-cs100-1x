...
def simpleTokenize(string):
    ...
    """
    token_list = re.split(split_regex,string.lower())
    filtered_token_list = [token for token in token_list if token != '']
...
def tokenize(string):
    ...
    """
    token_list = re.split(split_regex,string.lower())
    filtered_token_list = [token for token in token_list if token != '' and token not in stopwords]
    return filtered_token_list
...
def countTokens(vendorRDD):
    ...
    """
    tokenCount = vendorRDD.map(lambda (k,v): len(v)).reduce(lambda v1,v2: v1+v2)
    return tokenCount
...
def findBiggestRecord(vendorRDD):
    ...
    """
    biggestRecord = vendorRDD.sortByKey().takeOrdered(1,lambda v: -1 * len(v[1]))
...
def tf(tokens):
    ...
    tokenCountDict = {}
    for token in tokens:
        if token not in tokenCountDict:
            tokenCountDict[token] = 1
        else:
            tokenCountDict[token] = tokenCountDict[token] + 1
            
    tfDict = {}
    for token in tokenCountDict:
        tfDict[token] = tokenCountDict[token]/float(len(tokens))
...
def idfs(corpus):
    ...
    tokenSumPairTuple = tokenCountPairTuple.map(lambda k: (k,1)).reduceByKey(lambda v1,v2:v1+v2)
...
def cossim(a, b):
    ...
    na = norm(a)
    nb = norm(b)
    cosSimilarity = dp/(float(na)*float(nb))
...
def cosineSimilarity(string1, string2, idfsDictionary):
    ...
    """
    string1Tokens = tokenize(string1)
    string2Tokens = tokenize(string2)
    w1 = tfidf(string1Tokens,idfsDictionary)
    w2 = tfidf(string2Tokens,idfsDictionary)
    return cossim(w1, w2)
...
trueDupsRDD = (sims
               .join(goldStandard).map(lambda s: (s[0],s[1][0])))

trueDupsCount = trueDupsRDD.count()

avgSimDups = (trueDupsRDD.map(lambda s: s[1]).reduce(lambda v1,v2: v1+v2))/ float(trueDupsCount)
...
avgSimNon = (nonDupsRDD.map(lambda s: s[1]).reduce(lambda v1,v2: v1+v2))/ float(nonDupsCount)
...
def invert(record):
    ...
    """
    recordId = record[0]
    tokenWeightDict = record[1]
    pairs = []
    
    for token in tokenWeightDict:
        pairs.append((token,recordId))
    return (pairs)

amazonInvPairsRDD = (amazonWeightsRDD
                    .flatMap(lambda (element): invert(element))
                    .cache())
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    amazonTFIDFDict = amazonWeightsBroadcast.value[amazonRec]
    googleTFIDFDict = googleWeightsBroadcast.value[googleRec]
    s = 0
    for token in tokens:
        s = s + (amazonTFIDFDict[token] * googleTFIDFDict[token])
